...
export default function Experiments() {
  ...
  // Chart data: one point per batch with a score, labeled by batch index
            ...
            {[...batches].reverse().map((b) => (
              <tr key={b.batch_id} className="hover:bg-gray-900/40 transition-colors">
                ...
                <td className="px-4 py-3 text-gray-300 text-xs">
                  {b.goal_achievement_rate !== null
                    ? `${Math.round(b.goal_achievement_rate * 100)}%`
                    : "—"}
                </td>
                <td className="px-4 py-3">
                  {b.optimizer_accepted ? (
